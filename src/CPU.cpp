...
uint8_t CPU::read8(uint16_t addr) { return nes->cpuRead8(addr, false); }
uint16_t CPU::read16(uint16_t addr) { return nes->cpuRead16(addr, false); }
void CPU::write8(uint16_t addr, uint8_t data) { nes->cpuWrite8(addr, data); }
void CPU::write16(uint16_t addr, uint16_t data) { nes->cpuWrite16(addr, data); }
...
uint8_t CPU::ISB() {
    ...
    if (P & DECIMAL) {
        ...
        (((A ^ operand) & (A ^ difference)) & 128) ? setBit(OVERFLOW) :
            clrBit(OVERFLOW);
...
uint8_t CPU::RRA() {
    ...
    if (P & DECIMAL) {
        ...
        (((A ^ ah) & ~(A ^ operand)) & 128) ? setBit(OVERFLOW) :
            clrBit(OVERFLOW);
        if (ah > 0x9F) {
            ah += 0x60;
        }
...
uint8_t CPU::SBC() {
    ...
    if (P & DECIMAL) {
        ...
        (((A ^ operand) & (A ^ sub)) & 128) ? setBit(OVERFLOW) :
            clrBit(OVERFLOW);
        setZEROSIGN(sub);
        A = (ah << 4) | al;
    } else {
